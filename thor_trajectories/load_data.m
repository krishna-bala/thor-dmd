...
%%  Build X, X' and U matrices
%   Input: So
%   Output: n Segments of X, X' and U matrices
%   Leading and Trailing 25 frames are removed (for velocity averaging)


X = cell(1,trials); % 1 is a placeholder for the number of segments
X_prime = cell(1,trials);
U = cell(1,13,trials);
    ...
    for j = 1:13
        ...
        for k = 1:size(segment{11,i},1)
            ...
            if j == 11
                ...
                X_prime{k,i}(:,1)     = So{1,j,i}(start+26:stop-24)';
                
                X{k,i}(:,2)     = So{2,j,i}(start+25:stop-25)';
                X_prime{k,i}(:,2)     = So{2,j,i}(start+26:stop-24)';
                
                X{k,i}(:,3)     = So{3,j,i}(start+25:stop-25)';
                X_prime{k,i}(:,3)     = So{3,j,i}(start+26:stop-24)';
                
                X{k,i}(:,3)     = So{3,j,i}(start+25:stop-25)';
                X_prime{k,i}(:,3)     = So{3,j,i}(start+26:stop-24)';
                
                X{k,i}(:,5)     = So{5,j,i}(start+25:stop-25)';
                X_prime{k,i}(:,5)     = So{5,j,i}(start+26:stop-24)';
                
                X{k,i}(:,6)     = So{6,j,i}(start+25:stop-25)';
                X_prime{k,i}(:,6)     = So{6,j,i}(start+26:stop-24)';
